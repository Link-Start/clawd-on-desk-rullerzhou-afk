...
function detectRunningAgentProcesses(callback) {
  ...
  if (process.platform === "win32") {
    ...
    exec("pgrep -f 'claude-code|codex|copilot|codebuddy|kimi|@earendil-works/pi-coding-agent|pi-coding-agent/dist/cli\\.js' || pgrep -x 'gemini' || pgrep -x 'agy' || pgrep -x 'kiro-cli' || pgrep -x 'codewhale' || pgrep -x 'opencode' || pgrep -x 'mimo' || pgrep -x 'hermes' || pgrep -x 'qodercli' || pgrep -x 'qoder-cli' || pgrep -x '[Qq]oder[Ww]ork' || pgrep -f 'WorkBuddy Helper'", { timeout: 3000 },
      (err) => done(!err)
    );
  }
}
