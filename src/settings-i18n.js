...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      rowQuotaMergeSourcesDesc: "One set of gauges using the freshest live report for each quota window. Only for the same subscription on every machine - with different accounts per machine the merged numbers would lie.",
    ...
    zh: {
      ...
      rowQuotaMergeSourcesDesc: "只显示一组仪表，每个配额窗口取所有机器中最新的有效上报。仅适用于所有机器用同一订阅——各机器账号不同时合并数字会失真。",
    ...
    "zh-TW": {
      ...
      rowQuotaMergeSourcesDesc: "只顯示一組儀表，每個配額視窗取所有機器中最新的有效回報。僅適用於所有機器共用同一訂閱——各機器帳號不同時合併數字會失真。",
    ...
    ko: {
      ...
      rowQuotaMergeSourcesDesc: "각 할당량 기간마다 모든 머신에서 가장 최신의 유효한 보고를 골라 게이지 한 세트로 표시합니다. 모든 머신이 같은 구독일 때만 켜세요 - 머신마다 계정이 다르면 병합된 숫자가 부정확해집니다.",
    ...
    ja: {
      ...
      rowQuotaMergeSourcesDesc: "各クォータ期間について全マシン中の最新の有効な報告を選び、ゲージを1セットだけ表示します。全マシンが同じサブスクリプションの場合のみ有効にしてください - マシンごとにアカウントが異なると統合値は不正確になります。",
