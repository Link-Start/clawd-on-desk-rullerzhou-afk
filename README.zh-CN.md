...
- **Antigravity CLI (agy)** — 通过 `~/.gemini/config/hooks.json` 配置 command hook（已有 Antigravity 配置时 Clawd 启动会自动注册，或执行 `npm run install:antigravity-hooks`）；支持实时状态和实验性 Allow/Deny 权限气泡，不可用时回退到 agy 原生 `ask`
...
### 权限审批气泡
- **桌面端权限审批** — 当 Claude Code、Codex CLI、Antigravity CLI、CodeBuddy、opencode 或 Pi 请求工具权限时，Clawd 会弹出浮动卡片，无需切回终端
...
- **免打扰模式** — 右键或托盘菜单进入休眠，所有 hook 事件静默，直到手动唤醒。免打扰期间不弹权限气泡——Codex、Antigravity 和 opencode 会回退到原生命令行确认，Pi 会回退到终端确认，Claude Code 和 CodeBuddy 会回退到各自内置的权限确认流程
