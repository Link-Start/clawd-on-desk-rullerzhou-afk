...
- Antigravity CLI 通过 `~/.gemini/config/hooks.json` 做 hook 集成；`PreToolUse` 可接 Clawd Allow/Deny 权限气泡，DND / agent 禁用 / 气泡关闭 / Clawd 不可达时必须返回 `ask` 让 agy 原生确认接管；暂不写 `permissionOverrides` / Always Allow 规则
